addEventListener('load', (event) => {
    ...
    function fetchData(keyword, page = 1, pageSize = 10) {
        ...
        if (username && password) {
            fetch(`http://localhost:3000/jwTrainingAPI/training/search?username=${username}&password=${password}&keyword=${keyword}&page=${page}&pageSize=${pageSize}`)
                .then(response => { 
    ...
    function createCard(articleId, title, description, tags) {
        ...
        titleLink.href = 'http://localhost:3000/jwTrainingAPI/article?articleId=' + articleId;
